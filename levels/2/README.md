...
Palette: black / blue / cyan / white   (Asteroid Base)
...
LEVD3 is 2176 B (`&880`) — only the lower half of the LEVD2 layout is
overlaid. The map tile streams (`lev_map_upper` / `lev_map_lower`)
and the trailing gap are inherited from LEVD2 unchanged. Layout
from `&0000..&087F` matches the LEVD2 byte map above — that is, 14
enemy sprites + pointer tables + spawn tables + hazard sprites; everything
from `&0880` onwards keeps the LEVD2 bytes.
...
Stage 1: **55 spawn events** | Stage 2: **84 spawn events**
