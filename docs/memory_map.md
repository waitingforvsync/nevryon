...
| Hazard bullets       |   6 | `hazard_bullet_x[7]` (&1A8B), `hazard_bullet_y[7]` (&1A92); visible sprite at `hazard_bullet_sprite` (= &7E00, 4×2 px white) — poked by Loader2 line 1060 (`?&7E00 = ?&7E01 = 119`) | `LDX #n_hazard_bullets / DEX BNE` (slots 1..6) | Backing arrays are 7-wide; slot 0 is initialised to &FF by `init` but no routine ever touches it (predec loop falls off at X=0 before processing). Plot is a 3×2 read from `hazard_bullet_sprite` — col 0 = sprite, cols 1..2 = zero-pad trail eraser (bullets move LEFT 2 px/frame). |
