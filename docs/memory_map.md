...
| Player bullets      |   6 | `player_bullet_x[6]` (&16E6), `player_bullet_y[6]` (&16ED); visible sprite at `player_bullet_sprite` (= &7E06, 4×2 px palette-2 colour — yellow/cyan/green/magenta per scenario) — poked by Loader2 line 1060 (`?&7E06 = ?&7E07 = 112`) | `LDX #n_player_bullets / DEX BNE`  in `update_bullets` | Keyboard fire only scans slots 1..`n_player_fire_slots` (=4); slots 5..6 are reserved for the force-pod's twin shot (CODE2 `pod_fire` scans slots `n_player_bullets..n_player_bullets-n_player_fire_slots+1`). Plot is a 3×2 read from `player_bullet_sprite - 4` — cols 0..1 = zero-pad trail eraser, col 2 = sprite (bullets move RIGHT 2 px/frame). |
...
| NPC bullets       |   6 | `npc_bullet_x[7]` (&1A8B), `npc_bullet_y[7]` (&1A92); visible sprite at `npc_bullet_sprite` (= &7E00, 4×2 px white) — poked by Loader2 line 1060 (`?&7E00 = ?&7E01 = 119`) | `LDX #n_npc_bullets / DEX BNE` (slots 1..6) | Backing arrays are 7-wide; slot 0 is initialised to &FF by `init` but no routine ever touches it (predec loop falls off at X=0 before processing). Plot is a 3×2 read from `npc_bullet_sprite` — col 0 = sprite, cols 1..2 = zero-pad trail eraser (bullets move LEFT 2 px/frame). |
| NPC bullets (hz)  |   4 | (shares the pool above)                                                              | `LDX #n_enemy_npc_bullets / DEX BNE` (slots 1..4)              | Enemies fire (`enemy_try_fire_bullet`) only into the first 4 slots of the shared pool, leaving slots 5..6 for hazard-type-&04/&13 fires (`enqueue_npc_bullet`). Plus a global 8-frame cooldown at `zp_7E`. |
